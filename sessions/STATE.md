...
**Обновлено:** 2026-08-28, интеграция — Task 1 (мёрж `feature/map-routes` и
`feature/cabinet` в `main`, `feature/requests-moodboard` следующим шагом)
...
- Мёрж `feature/requests-moodboard` в `main` (последняя из трёх веток).
- Композиция `FavoriteButton` в экран карты и `SaveRouteButton` в сводку
  маршрута — по плану интеграции.
...
`feature/map-routes` и `feature/cabinet` смёржены в `main` (Task 1 плана
интеграции, `docs/superpowers/plans/2026-08-27-05-integration.md`).
Следующий шаг — смёржить `feature/requests-moodboard`, прогнать полный
набор проверок, затем Task 2 (композиция favorite/save-route в экран
карты).

**Bootstrap worktree (если создаётся новый):**
