...
**Обновлено:** 2026-08-28, интеграция — Tasks 1–8 из 10 завершены (мёрж,
композиция, дизайн-проход, golden path, изоляция, инъекция, независимый
аудит, агент-ломатель); осталось Task 9 (финализация) и Task 10 (чек-лист)
...
Полный набор проверок на объединённом дереве после Task 8 (агент-ломатель)
и всех его фиксов: `npm run build`, `npm run lint`, `npm run test`
(56/56), `npm run test:e2e` (6/6; периодически один транзиентный сбой
`cabinet-favorites.spec.ts` под 5–6-воркерной параллельной нагрузкой на
живой Supabase, не воспроизводится в изоляции и на повторном прогоне — см.
«Известные проблемы» ниже).

**Task 7 (независимый аудит) и Task 8 (агент-ломатель) завершены**,
отдельными свежими агентами (не форк), находки триажены и исправлены:

- Независимый аудит (`docs/independent-audit-report.md`): ни одно
  поведенческое утверждение из «Готово» не опровергнуто; единственная
  находка — сам `sessions/STATE.md` был внутренне рассинхронизирован
  (разделы «Не начато»/«Следующий шаг» отставали от «Готово»). Исправлено
  — этот файл синхронизирован целиком.
- Агент-ломатель (`docs/agent-breaker-report.md`, 21 попытка): 1 Critical
  + 5 Important находок, все исправлены (детали и коммиты — в
  `REPORT.md`, разделы «Агент-ломатель» и «Честно: что не сработало»):
  отсутствовавшие DB `CHECK`-ограничения на `places.lat/lng`,
  `requests.comment`, `routes/moodboards.place_ids` (клиентская валидация
  была единственной защитой — обходилась прямым `curl`); отсутствовавший
  submit-guard на «Сохранить мудборд» и «Сохранить маршрут в кабинет»
  (дубли строк при быстром клике); английские сообщения об ошибках
  Supabase Auth; неограниченная длина комментария заявки.
...
- Task 9 (финализация README/REPORT) и Task 10 (финальный чек-лист,
  секрет-скан, удаление worktree).
...
- `extractAverageColor` (мудборды) протестирован изолированно, но не
  ...
  `/add-place`.
- **Наблюдение при интеграции (Task 1 и повторно Task 8, полный
  `npm run test:e2e` с 4–6 параллельными воркерами против живого
  Supabase-проекта):** `cabinet-favorites.spec.ts` периодически падает с
  `new row violates row-level security policy for table "profiles"` при
  полном параллельном прогоне, но стабильно проходит и в изоляции
  (`npx playwright test e2e/cabinet-favorites.spec.ts`), и при повторном
  полном прогоне. Похоже на транзиентное ограничение живого
  Supabase-проекта под параллельной нагрузкой (та же природа, что уже
  наблюдалась раньше в этой сессии при интенсивном тестировании), а не
  регрессия кода — залогировано по инструкции интеграционного плана, тест
  не менялся и не удалён.
- **Найдено агентом-ломателем (Task 8), не исправлено за разумное время,
  задокументировано честно:** после входа в аккаунт (не регистрации) с
  уже существующими сохранёнными маршрутами «Мои маршруты» на `/cabinet`
  иногда на первом рендере показывает «Сохранённых маршрутов пока нет.»,
  хотя данные есть и подтверждены `curl`-ом — полная перезагрузка страницы
  сразу показывает маршруты корректно. `useMyRoutes.fetchRoutes` устроен
  так же, как уже работающий `useFavorites` (эффект, ключ `[session,
  isAnonymous]`), но конкретно на переходе «нет сессии → анонимная →
  зарегистрированная» именно во время sign-in иногда не подхватывает
  первый корректный fetch. Точная причина не найдена за разумное время;
  не блокирует MVP (данные не теряются, самоисправляется на reload).
- **Найдена узкая гонка при написании golden-path e2e-теста (Task 4),
  задокументирована честно, не спрятана:** `AuthProvider`'s
  `onAuthStateChange` реагирует на любое auth-событие, включая
  `USER_UPDATED`, которое supabase-js посылает синхронно как часть
  оптимистичного ответа `updateUser()` — **до** того, как явный вызов
  `refreshSession()` в `SignUpForm` (фикс из Task 7 этой ветки) успевает
  обновить сам JWT. Из-за этого `isAnonymous` в React-состоянии может
  стать `false`, и заголовок «Кабинет» — отрендериться, чуть раньше, чем
  JWT сессии реально станет нести `is_anonymous: false`. RLS-защищённая
  запись (например, добавление в избранное), отправленная в это узкое
  окно, может словить `403 42501`. Воспроизводимо скриптом с нулевым
  временем «раздумья» между действиями (golden-path тест ловил это
  стабильно, пока не научился ждать сетевой ответ `refresh_token`, а не
  только появление заголовка); маловероятно для живого человека,
  кликающего по интерфейсу. Не исправлено на уровне `AuthProvider` —
  осознанно отложено как низкоприоритетное (описание, не блокер),
  задокументировано здесь по правилу честности из спеки §14. Тест
  переписан так, чтобы явно ждать сетевой ответ обновления токена перед
  RLS-защищёнными действиями — `app/e2e/golden-path.spec.ts`, комментарий
  там же.

## Следующий шаг

Tasks 1–6 плана интеграции (`docs/superpowers/plans/2026-08-27-05-integration.md`)
завершены: мёрж трёх веток, композиция favorite/save-route (по I4 —
проверено эмпирически: react-leaflet монтирует `<Popup>` лениво, на 12
маркерах при загрузке ни одного запроса `/favorites`, `FavoritesProvider`
не понадобился), дизайн-проход `frontend-design` (скриншоты до/после в
`sessions/screenshots/`), golden-path e2e, демонстрация песочницы и
прогон инъекции (оба — Codex, включены в `REPORT.md`).

Task 7 (независимый аудит) и Task 8 (агент-ломатель) завершены — отчёты в
`docs/independent-audit-report.md` и `docs/agent-breaker-report.md`,
находки триажены и исправлены (см. выше и `REPORT.md`).

Следующий шаг — Task 9 (финализация README/REPORT.md, заполнить оставшиеся
плейсхолдеры) и Task 10 (финальный чек-лист: полный прогон проверок,
секрет-скан, удаление трёх feature-worktree, финальный коммит).
