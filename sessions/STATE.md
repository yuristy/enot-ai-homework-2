...
**Обновлено:** 2026-08-28, интеграция — Task 1 и Task 2 завершены (мёрж
трёх веток + композиция favorite/save-route в карту)
...
Task 1 (мёрж) и Task 2 (композиция) плана интеграции
(`docs/superpowers/plans/2026-08-27-05-integration.md`) завершены.
`FavoriteButton` вшит в попап маркера карты, `SaveRouteButton` — в
`RouteSummary`; проверено вручную через браузер полным golden path (гость
видит «Избранное недоступно без входа» → регистрация → избранное
сохраняется и видно в `/cabinet` → маршрут сохраняется и его ссылка из
`/cabinet` корректно открывает карту с тем же стартом и местом).

По I4 (дублирующиеся fetch'и `useFavorites()`) — проверено эмпирически, не
предположением: react-leaflet монтирует содержимое `<Popup>` лениво, по
одному, только при открытии попапа, и на 12 маркерах при загрузке карты
не происходит ни одного запроса `/favorites`; для анонимной сессии хук и
вовсе не делает запрос (short-circuit). Проблема параллельных запросов не
воспроизводится в этой точке композиции — `FavoritesProvider` не
понадобился.
