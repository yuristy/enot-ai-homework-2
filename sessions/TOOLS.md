...
- **Проверка:** `curl` к `/auth/v1/settings` вернул HTTP 200 с конфигом
  auth (валидная пара URL/ключ); схема (`schema.sql`), RLS-политики и
  триггер лимита (`policies.sql`) и seed (`seed.sql`) применены
  контроллером через Supabase SQL Editor и проверены `curl`-запросами:
  анонимный лимит 1/день срабатывает (повторная вставка в тот же день →
  `rate_limit_exceeded`), курируемые места (`source='curated'`) от лимита
  освобождены
